...
| Personal access tokens are available | Workspace configuration for `enableTokensConfig`; permissions on `authorization/tokens` | `enableTokensConfig` is `true` and the built-in `users` group has direct `CAN_USE` |
